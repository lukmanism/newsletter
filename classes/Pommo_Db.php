...
// TODO: $sql should always be passed by reference... look @ ways of doing this.
// ie. see Affected($sql) function
...
// Database Connection Class - holds the link, processes queries, produces
// reports, etc.
class Pommo_Db
{
    var $_link;
    ...
    // array holding unique results (for use with executing queries within loops
            ...
            $hostname = NULL, $tablePrefix = NULL, $compress = 0, $secure = 0)
    {
        ...
        // connect to mysql database using config variables from poMMo class
        // (set in setup/config.php).
        // supress errors to hide login information...
        $this->_link = mysql_connect(
            $hostname, $username, $password, false, $compress | $secure
        );

        if (!$this->_link)
            Pommo::kill(
                Pommo::_T('Could not establish database connection.')
                . ' '. Pommo::_T('Verify your settings in config.php')
            );
        ...
        if (!@ mysql_select_db($database, $this->_link))
            Pommo::kill(
                sprintf(
                    Pommo::_T(
                        'Connected to database server but could not '
                        . 'select database (%s). Does it exist?'
                    ), $database
                )
                . ' ' . Pommo::_T('Verify your settings in config.php')
            );
        ...
        //  code taken from Juliette Reinders Folmer;
        // http://www.adviesenzo.nl/examples/php_mysql_charset_fix/
        //  TODO: Cache the charset?
        $dbCharset = mysql_query(
            "SHOW VARIABLES LIKE 'character_set_database'",
            $this->_link
        );
        $charsetRow = mysql_fetch_assoc($dbCharset);
        mysql_query("SET NAMES '" . $charsetRow['Value'] . "'", $this->_link);
        unset ($dbCharset, $charsetRow);
    ...
    // if second parameter is not passed, pass a blank one (avoids safeSQL
    // throwing a warning)
    function prepare()
    {
        $a = func_get_args();
    ...
    function query($query, $row = NULL, $col = NULL)
    {
        ...
        if ($this->_debug) {
            ...
            $logger->addMsg(
                '[DB] Received query affecting '.$this->affected()
                . ' rows and returning '.$numRecords.' results. Query: '.$query
            );
        }
    ...
    // function affected - returns the amount of affects rows from a INSERT,
    // UPDATE, or DELETE Query.
    // Note, if nothing was changed in an update.. 0 will be returned!
    // if $sql is passed, a query will be issued. Otherwise examine the saved
    // result.
    ...
    // function records - returns the number of rows resultings in a SELECT
    // query -- 0 (false) if none...
    // if $sql is passed, a query will be issued. Otherwise examine the
    // saved result.
    ...
    // returns an array representing 1 row in a resultset. Returns an
    // assosiative array by default
    // this function pushes its result on a seperate stack, and is therefore
    // intended for LOOPs (while) ONLY
    ...
    */
    ...
    {
        // check for uniqueIdentifier (used to seperate result stacks when
        // embedding loops within loops)
        if ($uniqueId) {
            if (!isset ($this-> {
                '_' . $uniqueId }))
            $this-> {
        ...
        // Fetch row from result set at end of result stack
         ($enumerated) ? $row = mysql_fetch_row(end($set)) :
                $row = mysql_fetch_assoc(end($set));
    ...
    // retuns an entire result set as an associative array. pass type 'row' to
    // make it enumerated
    ...
    function & getAll($sql = NULL, $type = 'assoc', $field = NULL)
    {
        ...
        if ($field != NULL) {
            eval (
                'while(@$r = mysql_fetch_' . $type
                . '($this->_result)) array_push($a, $r[\'' . $field . '\']);'
            );
        } else {
            eval (
                'while(@$r = mysql_fetch_' . $type
                . '($this->_result)) array_push($a, $r);'
            );
        }
        return $a;
    }
